...
def collect_prob(data_loader, model, max_samples=None):
    """Collect model probabilities. If max_samples is set, stop after that many samples (faster MIA)."""
...
def SVC_MIA(shadow_train, target_train, target_test, shadow_test, model, max_samples=None):
    """Run SVC-based MIA. max_samples caps each loader at 1000 samples for speed (default). Set to None for full evaluation.

    Returns a dict of metrics on a **percentage** scale ``[0, 100]`` (internal rates are multiplied by 100).
    """
    shadow_train_prob, shadow_train_labels = collect_prob(shadow_train, model, max_samples=max_samples)
    shadow_test_prob, shadow_test_labels = collect_prob(shadow_test, model, max_samples=max_samples)

    target_train_prob, target_train_labels = collect_prob(target_train, model, max_samples=max_samples)
    target_test_prob, target_test_labels = collect_prob(target_test, model, max_samples=max_samples)
    ...
    m = {
        "correctness": float(acc_corr) * 100.0,
        "confidence": float(acc_conf) * 100.0,
        "entropy": float(acc_entr) * 100.0,
        "m_entropy": float(acc_m_entr) * 100.0,
        "prob": float(acc_prob) * 100.0,
    }
