...
def collect_performance(data_loader, model, device, max_samples=None):
    """Collect model outputs. If max_samples is set, stop after that many samples (faster MIA)."""
    probs = []
    labels = []
    model.eval()
    n_collected = 0
...
def MIA(
    ...
    """Run MIA. max_samples caps each loader at 1000 samples for speed (default). Set to None for full evaluation."""
    shadow_train_performance = collect_performance(retain_loader_train, model, device, max_samples=max_samples)
    shadow_test_performance = collect_performance(test_loader, model, device, max_samples=max_samples)
    target_train_performance = collect_performance(retain_loader_test, model, device, max_samples=max_samples)
    target_test_performance = collect_performance(forget_loader, model, device, max_samples=max_samples)
