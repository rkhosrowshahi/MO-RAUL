...
def svhn_dataloaders(
    ...
    elif num_indexes_to_replace is not None and indexes_to_replace is None:
        replace_class(
            train_set,
            -1,
            num_indexes_to_replace=num_indexes_to_replace,
...
def replace_class(
    ...
    # class_to_replace=-1 means "candidate pool = all samples". Without num_indexes_to_replace,
    # that pool is the full set, so every sample gets marked for forget and the retain split is empty
    # (DataLoader fails: num_samples=0). Require an explicit subset size for random forgetting.
    if class_to_replace == -1 and num_indexes_to_replace is None:
        raise ValueError(
            "class_to_replace=-1 with num_indexes_to_replace=None would mark every training "
            "sample as forget (no retain set). Set num_indexes_to_replace to the number of random "
            "samples to forget, set class_to_replace to a class id in [0, num_classes-1], or pass "
            "indexes_to_replace instead."
        )
...
class TinyImageNet:
    ...
    def data_loaders(
        ...
        train_transform = self.tr_test if no_aug else self.tr_train
        ...
        _generator = torch.Generator().manual_seed(seed) if seed is not None and shuffle else None
