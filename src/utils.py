...
def get_optimizer(model, args, lr=None):
    """
    Get optimizer based on args configuration.
    
    Args:
        model: The model to optimize
        args: Arguments containing optimizer configuration
        lr: Optional learning rate override (defaults to args.lr or args.unlearn_lr)
...
def warmup_lr(epoch, step, optimizer, one_epoch_step, args):
    ...
    # Unlearning (main_forget) uses unlearn_lr in the optimizer; pretrain uses args.lr.
    # Without this, default args.lr=0.1 overrides per-step warmup and explodes the LR vs unlearn_lr.
    max_lr = getattr(args, "warmup_max_lr", None)
    if max_lr is None:
        max_lr = args.lr

    lr = max_lr * current_steps / overall_steps
    lr = min(lr, max_lr)
...
def setup_model_dataset(args):
    if args.dataset == "cifar10":
        ...
        # Match upstream Unlearn-Saliency: data splits use --seed; model init uses --train_seed.
...
def setup_seed(seed):
    """Match upstream Unlearn-Saliency: torch, CUDA, NumPy, Python ``random``, cudnn deterministic."""
...
def setup_torch_seed(seed):
    """Set only torch/CUDA seeds; used for dataloader shuffle and other torch RNG."""
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)


def wandb_sanitize_group(group: str, max_len: int = 128) -> str:
    """W&B rejects group names longer than ``max_len`` characters (API limit)."""
    if not group or len(group) <= max_len:
        return group
    h = hashlib.sha256(group.encode("utf-8")).hexdigest()[:16]
    # prefix + "..." + hash, total length == max_len
    prefix_len = max_len - len(h) - 3
    return f"{group[:prefix_len]}...{h}"
...
def get_loader_from_dataset(dataset, batch_size, seed=1, shuffle=True):
    """Create DataLoader with reproducible shuffle order when shuffle=True."""
    g = torch.Generator()
    g.manual_seed(seed)
...
def apply_fixed_random_labels_to_forget_dataset(forget_dataset, num_classes, seed):
    """Assign fixed random labels to the forget split (reproducible via ``seed``).

    Each random label is drawn uniformly from the wrong classes only (never equal
    to that sample's true label). For class ``t``, this is uniform on
    ``{0,...,num_classes-1} \\ {t}``.

    Used for SalUn / random-label-style objectives: one label vector per run, not
    refreshed each batch (see ``RL`` for per-batch random labels in SGD).
    """
    if num_classes < 2:
        raise ValueError(
            "apply_fixed_random_labels_to_forget_dataset requires num_classes >= 2 "
            "so random labels can differ from the true label"
        )
    rng = np.random.RandomState(int(seed))

    def _wrong_class_labels(true: np.ndarray) -> np.ndarray:
        """Uniform random label in [0, num_classes) \\ {true[i]} per row."""
        n = len(true)
        j = rng.randint(0, num_classes - 1, size=n)
        rnd = np.where(j < true, j, j + 1)
        assert np.all(rnd != true), "random labels must not match true labels"
        return rnd

    if hasattr(forget_dataset, "targets"):
        true = np.asarray(forget_dataset.targets, dtype=int)
        forget_dataset.targets = _wrong_class_labels(true).tolist()
    elif hasattr(forget_dataset, "labels"):
        true = np.asarray(forget_dataset.labels, dtype=int)
        forget_dataset.labels = _wrong_class_labels(true).tolist()
    else:
        raise ValueError(
            "forget_dataset must expose .targets or .labels for random relabeling"
        )


def assert_forget_retain_partition_indices(marked_dataset):
    """Verify training indices partition into forget (target < 0) and retain (target >= 0).

    Raises AssertionError if any index is in both sets, in neither set, or counts do not match.
    """
    if hasattr(marked_dataset, "targets"):
        t = np.asarray(marked_dataset.targets)
    elif hasattr(marked_dataset, "labels"):
        t = np.asarray(marked_dataset.labels)
    else:
        raise ValueError("marked_dataset must expose .targets or .labels")
    n = len(t)
    forget_idx = np.flatnonzero(t < 0)
    retain_idx = np.flatnonzero(t >= 0)
    overlap = np.intersect1d(forget_idx, retain_idx, assume_unique=True)
    if overlap.size > 0:
        raise AssertionError(
            f"forget/retain share {overlap.size} training indices (first 20): {overlap[:20]!r}"
        )
    all_idx = np.union1d(forget_idx, retain_idx)
    neither = np.setdiff1d(np.arange(n, dtype=all_idx.dtype), all_idx)
    if neither.size > 0:
        raise AssertionError(
            f"{neither.size} training indices are neither forget nor retain (first 20): {neither[:20]!r}"
        )
    if forget_idx.size + retain_idx.size != n:
        raise AssertionError(
            f"index count mismatch: |forget|+|retain|={forget_idx.size + retain_idx.size}, n={n}"
        )
